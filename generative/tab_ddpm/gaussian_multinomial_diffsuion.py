...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def mixed_loss_unlabeled(self, x, out_dict):
        b = x.shape[0]
    ...
    @torch.no_grad()
    def sample(self, num_samples, y_dist, verbose=True):
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.num_numerical_features), device=device)
    ...
    def sample_all(self, num_samples, batch_size, y_dist, ddim=False, verbose=True):
        if ddim:
        ...
        while num_generated < num_samples:
            sample, out_dict = sample_fn(b, y_dist, verbose=verbose)
