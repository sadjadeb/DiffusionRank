...
class MLP1D(nn.Module):
    def __init__(self, input_dim, hidden_dims=(256, 512, 256), output_dim=None, self_condition=False):
        super().__init__()
        
        self.self_condition = self_condition
        input_size = input_dim * (2 if self_condition else 1)
        ...
        self.output_layer = nn.Linear(prev_dim, output_dim if output_dim else input_dim)
...
class GaussianDiffusion1D(Module):
    ...
    def denoise(self, noisy_seq):
        ...
        denoised_seq = noisy_seq.clone()
        x_start = None
        for t in tqdm(reversed(range(self.num_timesteps)), desc = 'denoising time step', total = self.num_timesteps):
...
class GaussianDiffusionMLP1D(Module):
    def __init__(
        self,
        model,
        *,
        ...
        assert objective in {'pred_noise', 'pred_x0', 'pred_v'}, (
            "Objective must be one of 'pred_noise', 'pred_x0', or 'pred_v'"
        )
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        if self.self_condition and random() < 0.5:
            with torch.no_grad():
                x_self_cond = self.model_predictions(x, t).pred_x_start.detach()

        model_out = self.model(x, t, x_self_cond)
        target = noise if self.objective == 'pred_noise' else (
            x_start if self.objective == 'pred_x0' else self.predict_v(x_start, t, noise)
        )
    ...
    def forward(self, img, *args, **kwargs):
        b, _, n, device = *img.shape, img.device
