...
# Convert to torch tensors (training data is kept as numpy for perturbation)
X_train = torch.from_numpy(X_train).float().to(device)
bin_y_train = torch.from_numpy(bin_y_train).long().to(device)
...
if __name__ == '__main__':
    ...
    if args.task == 'train':
        ...
        for epoch in range(num_epochs+1):
            ...
            if epoch > 0:
                ...
                train_reader = torch.utils.data.TensorDataset(x_noisy, bin_y_train)
        ...
        final_model_save_path = os.path.join(project_root, 'discriminative', 'checkpoints', f'ltr.{dataset}.pointwise_perturbed.k{k}.{args.exp_name}.final.pt')
        best_model_save_path = os.path.join(project_root, 'discriminative', 'checkpoints', f'ltr.{dataset}.pointwise_perturbed.k{k}.{args.exp_name}.best.pt')
    ...
    # Save results
    results_save_path = os.path.join(project_root, 'discriminative', 'predictions', f'ltr.{dataset}.pointwise_perturbed.k{k}.{args.exp_name}.best.txt')
