...
parser = argparse.ArgumentParser(description='Train and test a pairwise neural network for Learning-to-Rank tasks')
...
def generate_pairs(qid_data, batch_size, device):
    """
...
def train(net):
    ...
    for _ in range(num_batches_per_epoch):
        features_i, features_j = generate_pairs(train_data_by_qid, batch_size, device)
...
if __name__ == '__main__':
    ...
    if args.task == 'train':
        ...
        for epoch in range(num_epochs+1):
            ...
            print(f'epoch: {epoch}, train_loss: {train_loss}, val_p: {val_metrics["p"]}, val_ndcg: {val_metrics["ndcg"]}, test_p: {test_metrics["p"]}, test_ndcg: {test_metrics["ndcg"]}')
        ...
        final_model_save_path = os.path.join(project_root, 'discriminative', 'checkpoints', f'ltr.{dataset}.pairwise.k{k}.{args.exp_name}.final.pt')
        best_model_save_path = os.path.join(project_root, 'discriminative', 'checkpoints', f'ltr.{dataset}.pairwise.k{k}.{args.exp_name}.best.pt')
    ...
    test_metrics = test(net, test_reader_iter)
    print(f'Best Model Performance: test_p: {test_metrics["p"]}, test_ndcg: {test_metrics["ndcg"]}')
        
    # Save results
    results_save_path = os.path.join(project_root, 'discriminative', 'predictions', f'ltr.{dataset}.pairwise.k{k}.{args.exp_name}.best.txt')
